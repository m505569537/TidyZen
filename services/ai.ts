...
3. 椅子/沙发 — 堆放的衣物、包包、杂物
4. 床铺 — 散乱的衣物、杂物；明显凌乱（揉成团、堆积未铺、歪斜散落）的被子/枕头计为 pillow_blanket，整齐叠放/正常铺好的不算
...
3. 不要把家具(床、桌、椅、柜)、书架、墙上的画/海报、装饰品当成杂物。正常铺好的被子/枕头是家具/用品不算杂物，但明显凌乱揉成一团的被褥算杂物（pillow_blanket）。
...
11. **场景判定优先级**：如果画面是房间整体视角、能看到床/沙发/书桌/浴室柜等主要家具，scene 优先填 bedroom | living_room | bathroom | desk_area；只有画面完全是地面特写（无任何家具出现，或家具只占极小边角）才填 floor。杂物堆在墙角/地面不改变房间类型判断。`;
