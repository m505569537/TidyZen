...
> 最后更新: 2026-08-12（新增按钮/数据真实性审计，详见 §八、§九 + `docs/BUTTON_AUDIT.md`）
...
4. **账号安全页** — Alert 占位（5 个按钮），5 个都需要后端，详见 §九
5. **通知偏好** — 本地状态，没有真实推送
6. **🔴 20 个死按钮**（2026-08-12 新发现）— 详见 `docs/BUTTON_AUDIT.md` §1，包括主页"操作"/"查看全部"、history 3 个导航按钮、profile 2 个保存/换头像、camera 闪光灯、help/privacy/about 多个 mailto 按钮、record 分享、result info 按钮等
7. **🔴 5 处假数据**（2026-08-12 新发现）— `profile.tsx` 昵称/性别不持久化（切走就丢）、称号/等级硬编码、settings 缓存大小写死 "124 MB"
8. **🟡 ≥1 导航孤岛**（2026-08-12 新发现）— `/change-password` 和 `/terms` 页面存在但 `router.push` 0 引用

## 九、真空按钮池（2026-08-12 新增 — 见 `docs/BUTTON_AUDIT.md`）

| 类别 | 数量 | 可本地修 | 必须后端 |
|---|---|---|---|
| 死按钮 | 20 | 全部（加 Alert 或实现） | 0 |
| Alert 占位 | 9 | 3（已存在页面跳转+退出登录清状态） | 6（账号类） |
| 假数据 | 5 | 5（接 AsyncStorage 或动态算） | 0 |
| 导航孤岛 | ≥1 | 1（补入口或删页） | 0 |
| **合计** | **35** | **29** | **6** |

**说明**：
- §九 描述的 9 个 Alert 占位里 3 个本地能立即修：settings 房间模板/建议库偏好（页面已存在，5 分钟）+ 退出登录（清 store+storage，1-2 小时）
- §七 待办里 #5 账号安全页 5 个按钮"等后端"：保持 Alert 占位合规
- 死按钮全部 20 个本地可修：30 分钟批量加 Alert 保 App Store 审核，或 1-1.5 天按建议修法实现
- 详见 `docs/BUTTON_AUDIT.md` §6 修复路径

## 十、产品方向 (2026-06-22 Grill 总结)
