...
### 待做功能
- [ ] Before/After 对比 — 重新扫描时保存上一张照片，并排对比 + 分数变化
- [ ] 房间模板生效 — 拍照前选场景，影响 AI 分析和建议
- [ ] 快速扫描 vs 精准扫描 — scan.tsx 双入口（快速=自动判断场景，精准=先选场景再拍）
- [ ] 成就系统 — 勋章墙触发逻辑（连续扫描、分数提升等）
- [ ] 数据持久化 — iCloud 同步（先做），后端服务（后做）
- [ ] 视频教程 — 教用户怎么整理（自己拍），不是教用 App
- [ ] 建议库预留 sponsor 字段 — 为品牌合作做准备
