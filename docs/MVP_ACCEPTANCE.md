...
> 更新日期：2026-08-12（按钮审计刷新：发现 20 死按钮 + 5 假数据，比 §7.4 旧统计多 27 个）
...
| 7.2 用户体验 | **1** | 2 | 1 | 0 |
| 7.3 性能指标 | **1** | 1 | 1 | 0 |
| 7.4 按钮响应 | 8 | 0 | **20** ⚠️ 8/12 旧 | 0 |
| 7.5 数据真实性 | 0 | 0 | **5** | 0 |
| 8.1 核心埋点 | **6** | 0 | **0** | 0 |
| **合计** | **18** | **3** | **30** | 0 |

**MVP 当前真实状态**：⚠️ **接近可发布但有 30 个❌**（18 ✅ / 3 ⚠️ / 30 ❌，2026-08-12 按钮审计后）

> ⚠️ **2026-08-12 重大刷新**：用户指出 settings 页"好多按钮没实现"，经完整代码审计发现 20 死按钮 + 9 Alert 占位 + 5 假数据 + ≥1 导航孤岛 = 35 处问题（比之前漏报 27 个）。详见 §7.5 与 `docs/BUTTON_AUDIT.md`。
...
## 6. §7.5 按钮 / 数据真实性审计（2026-08-12 新增，重大发现）

> 来源：完整代码 grep + 逐行读，**非**文档推断。完整清单见 [`docs/BUTTON_AUDIT.md`](BUTTON_AUDIT.md)。

### 6.1 发现总览

| 类别 | 数量 | 严重度 | 之前漏报 |
|---|---|---|---|
| **A. 死按钮**（无 onPress） | **20** | 🔴 App Store 红线 | 漏 12 |
| **B. Alert 占位**（弹"开发中"） | **9** | 🟡 已合规 | 漏 1 |
| **C. 假数据**（硬编码冒充） | **5** | 🟡 严重误导 | 漏 5 |
| **D. 导航孤岛** | **≥1** | 🟡 隐藏功能 | 漏 1 |
| **合计** | **35** | — | **漏 19** |

### 6.2 §7.4 旧统计与现状对照

| 类别 | §7.4 旧版 | 2026-08-12 实际 | 差距 |
|---|---|---|---|
| 死按钮 | 8 (account 5 + settings 3) | **20** | +12 |
| Alert 占位 | 8 | **9** | +1 |
| 假数据 | 0 | **5** | +5 |
| 导航孤岛 | 0 | **≥1** | +1 |

**§7.4 仅覆盖了 account + settings 两个文件的"开发中"Alert，忽略了其他 12 个文件的死按钮、5 处硬编码假数据、1 个导航孤岛。** 详见 BUTTON_AUDIT.md §1-§4。

### 6.3 哪些能立刻修（本地能做的）

- **死按钮方案 1（30 分钟）**：所有 20 个加 Alert 兜底，保住 App Store 审核
- **死按钮方案 2（1-1.5 天）**：按 BUTTON_AUDIT.md §6 推荐顺序实现
- **假数据 5 处（1-2 小时）**：profile 头像/昵称/性别 + settings 等级/缓存
- **导航孤岛 1 处**：补 `/change-password` 入口或删除（待用户拍板）
- **退出登录（1-2 小时）**：清 store/storage + 回首页，目前弹 Alert

### 6.4 哪些必须等后端（5 个）

- 修改手机号、微信绑定/解绑、邮箱绑定、修改密码、注销账号
- 现状：Alert 占位，**已合规不阻塞上架**

### 6.5 影响判定

- **App Store 审核**：死按钮 20 个是 Guideline 2.2 红线，必须修
- **用户信任**：假数据 5 处严重误导（昵称改了不存 = "保存"按钮是骗人）
- **修复总成本**：本地可做部分 ≈ 1-1.5 天
...
### 7.4 ⚠️ 2026-08-12 审计刷新：§7.4 旧统计不完整
- **§7.4 旧版只统计了 account + settings 2 个文件**，**漏报 12 个死按钮、5 处假数据、≥1 导航孤岛**
- 完整清单 35 处问题见 [`BUTTON_AUDIT.md`](BUTTON_AUDIT.md)
- 旧"8/8 按钮全部有 onPress ✅"是错的（其他 12 个文件未审计）
